<template>
  <div>
    <component v-bind:is="tsxDemo"></component>
...
export default class DynamicForm extends Vue {
  ...
  private title: string= 'hello tsx';

  get tsxDemo() {
      return {
        render: () => <div>{this.title}</div>
      };
    }
  ...
  get formItems(){
    const items = <a-form-item label="标题">
              <a-input placeholder="给目标起个名字" />
            </a-form-item>;
    const item =([
      this.buildFormitem({type:'string',label:'测试'}),this.buildFormitem({type:'string',label:'测试2'})
    ]);
